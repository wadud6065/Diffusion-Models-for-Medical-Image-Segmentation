...
class MyDataLoader(Dataset):
    ...
    def transform(self, noisy_image, clean_image):
        ...
        noisy_image, clean_image = noisy_image.type(torch.FloatTensor), clean_image.type(torch.FloatTensor)
        ...
        while cnt_try < 10 and index < self.__len__():
            try:
                ...
                noisy_image, clean_image = self.transform(
                    noisy_image, clean_image)
                return noisy_image, clean_image, self.noisy_image_path[index], self.clean_image_path[index]
